...
for t in range(0, videos_without_rois_test.shape[0]):
    ...
    if t == 143:
        ad = 3
    fgmask = fgbg.apply(videos_without_rois_test[t, :, :, :])

    fgmask = cv2.morphologyEx(fgmask, cv2.MORPH_ERODE, kernel)

    keypoints = detector.detect(fgmask)

    im_with_keypoints = cv2.drawKeypoints(videos_without_rois_test[t, :, :, :], keypoints, np.array([]), (0, 0, 255),
